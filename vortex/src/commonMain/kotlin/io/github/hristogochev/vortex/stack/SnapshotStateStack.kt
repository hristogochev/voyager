...
import io.github.hristogochev.vortex.util.takeUntilInclusive
...
public class SnapshotStateStack<Item>(
    ...
    public override var current: Item
        get() = lastItem
        set(value) {
            replace(value)
        }

    override val lastItem: Item by derivedStateOf {
        stateStack.lastOrNull() ?: error("Stack is empty")
    }
    ...
    public override fun replaceAllWith(predicate: (List<Item>) -> List<Item>) {
        Snapshot.withMutableSnapshot {
            val modItems = predicate(stateStack)
            stateStack.clear()
            stateStack += modItems
            lastEvent = StackEvent.Replace
        }
    ...
    public override fun replaceUntil(item: Item, predicate: (Item) -> Boolean) {
        Snapshot.withMutableSnapshot {
            val modItems = stateStack.takeUntilInclusive(predicate) + item
            stateStack.clear()
            stateStack += modItems
            lastEvent = StackEvent.Replace
        }
    ...
    public override fun replaceUntil(items: List<Item>, predicate: (Item) -> Boolean) {
        Snapshot.withMutableSnapshot {
            val modItems = stateStack.takeUntilInclusive(predicate) + items
            stateStack.clear()
            stateStack += modItems
            lastEvent = StackEvent.Replace
        }
